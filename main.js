...
const PhaserGame = function (game) {
    ...
    this.panelDimensions.width = this.gridsize * 6;
    this.panelDimensions.x = SCREEN_WIDTH - this.panelDimensions.width;
    this.panelDimensions.y = 0;

    this.statusBar = {};
    this.statusBarDimensions = {};
    this.statusBarDimensions.width = this.gridsize * 6;
    ...
    this.worldWidth = (this.gridsize * this.tilesHigh) + this.panelDimensions.width;
    this.worldHeight = (this.gridsize * this.tilesHigh);
};
...
PhaserGame.prototype = {
	...
	// panel + statusbar
	const { x, y } = this.panelDimensions;
	this.panel = this.add.sprite(x, y, 'panel');
	...
	for (let attribute in this.heroAttributes) {
	    ...
	    const { x, y, width, height, padding } = this.statusBarDimensions;
	    ...
	    this.statusBar[attribute].fixedToCamera = true;
	...
	this.resetAttack();
	this.camera.focusOnXY(this.hero.x+(this.panelDimensions.width/2), this.hero.y);
    },
	...
	if (this.time.now > this.attackTime)
	{
	    if (this.attacks)
	    {
		this.attacks.reset(this.activeTile.worldX+(this.gridsize/1), this.activeTile.worldY+(this.gridsize/2));
